...
const express = (Apper && Apper.express) ? Apper.express : require('express');
const ws = require('ws');
...
function getConfiguredHosts() {
  ...
  if (config.commonName) {
    hosts.add(config.commonName.replace(/:\d+$/, '').replace(/^\[|\]$/g, '').trim().toLowerCase());
  }
  for (const rule of rules) {
    if (rule.getFromHost()) {
      hosts.add(rule.getFromHost().replace(/:\d+$/, '').replace(/^\[|\]$/g, '').trim().toLowerCase());
    }
  }
  return hosts;
}
...
function isAllowedHost(req) {
  ...
  const host = rawHost.replace(/:\d+$/, '').replace(/^\[|\]$/g, '').trim().toLowerCase();
  if (net.isIP(host)) {
    return false;
  }
...
app.use((req, res, next) => {
  const remoteAddress = req.socket ? req.socket.remoteAddress : undefined;
  res.on('finish', () => {
...
module.exports = async function() {
  try {
    ...
    logger.error(e);
    if (typeof logger.on === 'function' && typeof logger.end === 'function') {
      logger.on('finish', () => {
        process.exit(1);
      });
      logger.end();
